...
for po_config in "${po_configs[@]}"; do
  ...
  for i in 1 2 3; do
    ...
    bash run-tpch-q-adapt.sh -n $name -q "${tid} ${qid}" -c "${k1} ${k2} ${k3} $((k2 * k3 * 2)) 48m 200 true 0.6" -p "${s1} 0.2 0b ${s4} ${s5} 256MB 5.0 128MB 4MB" -x "outs/taskd" -i ${uId} | tee logs/${name}_x${i}.log 2>&1
  done
done